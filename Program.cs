...
namespace Midi_Counter_Generator
{
    ...
    class PairliComparer : IComparer
    {
    ...
    class PairlsComparer : IComparer
    {
        public int Compare(object a, object b)
        {
            pairls fx = (pairls)a;
    ...
    class mainpart
    {
        ...
        void OriginalRender()
        {
            ...
            // QUICK SORT FIX: Applies the fast sorting algorithm here
            Console.WriteLine("Sorting tempo events");
            bpm.Sort(new PairliComparer());
        ...
        void FastRender()
        {
            ...
            // Fixed OOM: Using Dictionaries instead of padding endless lists
            Dictionary<int, int> nts = new Dictionary<int, int>();
            Dictionary<int, int> nto = new Dictionary<int, int>();
            ...
            for (int trk = 0; trk < trkcnt; trk++)
            {
                ...
                int lstcmd = 256;
                Console.WriteLine("Reading track {0}/{1}, Size {2}", nowtrk, trk + 1, trkcnt, leng);
                int getnum()
                {
                    int ans = 0;
                    int ch = 256;
                ...
                while (true)
                {
                    ...
                    if (cm == 0b10010000)
                    {
                        ...
                        if (!nts.ContainsKey(TM)) nts[TM] = 0;
                        nts[TM]++;
                        notes++;
                    }
                    else if (cm == 0b10000000)
                    {
                        ...
                        if (!nto.ContainsKey(TM)) nto[TM] = 0;
                        nto[TM]++;
                    }
                    else if (cm == 0b11000000 || cm == 0b11010000 || cmd == 0b11110011)
                    {
            ...
            // QUICK SORT FIX: Applies the fast sorting algorithms here
            Console.WriteLine("Sorting tempo events");
            bpm.Sort(new PairliComparer());

            Console.WriteLine("Sorting lyrics events");
            lrcs.Sort(new PairlsComparer());
            ...
            while (now <= alltic)
            {
                for (; now <= tmm; now++)
                {
                    ...
                    int nts_val = 0;
                    int nto_val = 0;
                    nts.TryGetValue(now, out nts_val);
                    nto.TryGetValue(now, out nto_val);
